...
import 'package:backend/src/modules/auth/errors/auth_errors.dart';
import 'package:backend/src/modules/auth/guard/auth_guard.dart';
import 'package:backend/src/modules/auth/repositories/auth_repository.dart';
...
class AuthResource extends Resource {
  ...
  List<Route> get routes => [
        ...
        Route.get("/refresh_token", _refreshToken, middlewares: [AuthGuard(isRefreshToken: true)]),
        Route.get("/check_token", _checkToken, middlewares: [AuthGuard()]),
        Route.put("/update_token", _updatePassword, middlewares: [AuthGuard()]),
      ];
  ...
  FutureOr<Response> _checkToken(Injector injector, Request request) {
    return Response.ok(jsonEncode({'message': 'ok'}));
  }

  
  FutureOr<Response> _updatePassword(Injector injector, Request request, ModularArguments arguments) async {
    final extractor = injector.get<RequestExtractor>();
    final authRepository = injector.get<AuthRepository>();
